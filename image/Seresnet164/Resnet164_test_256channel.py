...
import numpy as np
#from TTA-wrapper-master.src import ClassPredictor
import os
import sys
import time
import argparse
...
parser.add_argument('--ckpt_path', default='./logs/June02  17-01-15-labelweight-0.36/last_ckpt', type=str, metavar='PATH',
...
#20000-30000_data
    ...
    if args.resume:
        ...
        model = models.__dict__[args.arch](num_classes)
        #pretrained_dict = torch.load('/home/mfx/.torch/models/resnet101-5d3b4d8f.pth')   #pretrained_model_path，这是我下载的模型文件
        #model_dict=model.state_dict()
        #model_dict.update({k: v for k, v in pretrained_dict.items() if k in model_dict})
        #model.load_state_dict(model_dict)
        #model.load_state_dict(torch.load('./logs/May31  18-42-58-0.4407-0.5mecifar/last_ckpt/20.pth'))
    ...
    if(nottrained):
        for epoch in range(start_epoch, args.epochs):
            ...
            if(epoch%5==0):
                torch.save(model.state_dict(), os.path.join(log_dir, 'last_ckpt', str(epoch)+'.pth'))  
                eval(val_loader, model, criterion, test_writer, start_epoch, 12,nottrained)
    else:
        nottrained=False;        
        eval(test_loader,test_loader3, model, criterion, test_writer, start_epoch, 12,nottrained)        
    train_writer.close()
    test_writer.close()
...
def adjust_learning_rate(optimizer, epoch):
    if args.lr_schedule == 0:
        lr= args.lr * ((0.1 ** int(epoch >= 60)) * (0.1 ** int(epoch >= 120)) * (0.1 ** int(epoch >= 160)))#第一个可以考虑改成30,第二个改成  太多会导致过拟合
...
# Evaluating
def eval(test_loader,test_loader3, model, criterion, writer, epoch, num_iter,nottrained):
    ...
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        if torch.cuda.is_available():
            inputs, targets = inputs.cuda(), targets.cuda()
        outputs = model(inputs)
        output_value, predicted = torch.max(outputs.data, 1)
        correct = predicted.eq(targets.data).cpu().sum()
        acces.update(correct, inputs.size(0))
        ...
        if(nottrained==False):
            ...
            with open('./test_result_data.txt', "a", encoding="utf-8")as f:
                temp = AreaID + "\t" +'00'+str(CategoryID+1) + "\n"
                f.write(temp)  
            with open('./result_Probability.txt', "a", encoding="utf-8")as f:
                list_data=''
                for i in range(256):#导出256channel
                    list_data+="\t"+str(outputs.data[0][i].item())
                temp2 = str(000000+int(AreaID))+list_data+ "\t" +'00'+str(CategoryID+1)+"\n"
                f.write(temp2)   
